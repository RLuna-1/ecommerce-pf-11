...
async function getUsers(req, res) {
    ...
    try {
        ...
        if (filteredUsers.length === 0) {
            ...
            const updatedFilteredUsers = updatedUsers.filter(user => !user.disabled);
...
async function updateUserHandler(req, res) {
  ...
  async function deleteUserHandler(req, res) {
    try {
      ...
      res.status(500).json({ message: 'Ocurrió un error al desactivar el usuario' });
    }
  }

module.exports = { getUsers, getUserById, createUser, deleteUserHandler, updateUserHandler }